import {
    ...
    Material,
    Property as cesiumProperty,
    SceneTransforms,
} from "cesium";
//构建鼠标提示
export function MovePrompt(viewer,opt) {
    if (!opt) opt = {};
    ...
    if (opt.type && opt.type != 1 && this.popupCartesian) {
        ...
        this.rendHandler = this.viewer.scene.postRender.addEventListener(function () {
            if (that.popupCartesian) {
                var px = SceneTransforms.wgs84ToWindowCoordinates(that.scene, that.popupCartesian);
                ...
                if (!(n += 1 * that.scene.globe.ellipsoid.maximumRadius, Cartesian3.distance(i, e) > n)) {
                    res = true;
                }
                if (res && that.show) {
