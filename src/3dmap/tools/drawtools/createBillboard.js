...
export function CreateBillboard(viewer, style) {
	...
	this.image = style.image || "./images/blueMarker.png";
	this.style = style;
	this.billboardArg = {};
	this.billboard = null;
	if (style) {
...
CreateBillboard.prototype = {
	...
	createByPositions: function(lnglatArr, callBack) { 
		if (!lnglatArr) return;
		var position = Cartesian3.fromDegrees(lnglatArr[0], lnglatArr[1],lnglatArr[2]);
		if (!position) return;
	...
	createBillboard: function(cartesian) {
		...
		var billboard = this.viewer.entities.add({
			...
			billboard: {
				...
				heightReference: this.style.heightReference || HeightReference.NONE,
