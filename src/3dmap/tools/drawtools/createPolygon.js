...
export function CreatePolygon(viewer, style) {
	...
	start: function(callBack) {
		...
		this.handler.setInputAction(function(evt) { //移动时绘制面
			...
			if (that.positions.length == 2) {
				if (!defined(that.polyline)) that.polyline = that.createPolyline();
			} else {
	...
	createPolygon: function() {
		...
		return this.viewer.entities.add({
			polygon: {
				...
				material: this.style.material || Color.WHITE,
				width: this.style.width || 3,
				outlineColor: this.style.outlineColor || Color.BLACK,
