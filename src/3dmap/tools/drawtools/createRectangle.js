...
//绘制矩形
export function CreateRectangle(viewer, style) {
...
CreateRectangle.prototype = {
    start: function (callBack) {
        ...
        this.handler.setInputAction(function (evt) { //移动时绘制线
            ...
            that.radius = Cartesian3.distance(cartesian, that.leftup);
    ...
    startModify: function (callback) {
        ...
        if (!this.modifyHandler) this.modifyHandler = new ScreenSpaceEventHandler(this.viewer.scene.canvas);
        var that = this;
        ...
        this.modifyHandler.setInputAction(function (evt) {
            ...
            that.radius = Cartesian3.distance(that.rightdown, that.leftup);
        }, ScreenSpaceEventType.MOUSE_MOVE);
        ...
        var rectangle = this.viewer.entities.add({
            rectangle: {
                coordinates: new CallbackProperty(function () {
                    return Rectangle.fromCartesianArray([that.leftup, that.rightdown])
                }, false),
                material: this.style.material || Color.YELLOW,
                width: this.style.width || 3,
                heightReference: this.style.heightReference == undefined ? HeightReference.CLAMP_TO_GROUND : this.style.heightReference
            }
        });
